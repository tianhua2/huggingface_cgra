...
class Qwen2MoeAttention(nn.Module):
    ...
    def __init__(self, config: Qwen2MoeConfig, layer_idx: Optional[int] = None):
        ...
        self.q_proj = nn.Linear(self.hidden_size, self.num_heads * self.head_dim, bias=config.attention_qkv_bias)
        self.k_proj = nn.Linear(
            self.hidden_size, self.num_key_value_heads * self.head_dim, bias=config.attention_qkv_bias
        )
